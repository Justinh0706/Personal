...
try {
    ...
    if ($OutputYN -eq "Y") { 
        ...
        } ### NIEUWE ACCCOLADE HIER TOEGEVOEGD ###
    ...
    try {
        ...
        Write-Warning "Fout bij permanent verwijderen van de groep uit Entra ID prullenbak via Microsoft Graph (algemene catch): $($_.Exception.Message)"
        Write-Warning "Controleer handmatig of de groep en de bijbehorende SharePoint site volledig zijn verwijderd."
    }
    ...
    Write-Warning "BELANGRIJK: Zorg ervoor dat het account waarmee u nu inlogt (indien gevraagd) minimaal de rol 'SharePoint Administrator' heeft in Microsoft 365. Voltooi eventuele inlogprompts." # Oorspronkelijke waarschuwing
    ...
    while($siteStillExists -and $retryCountForClearance -lt $maxRetryForClearance) { # <- Typefout gecorrigeerd hier: $maxRetryFor Clearance -> $maxRetryForClearance
        $siteStillExists = $false # Reset voor deze iteratie, wordt true als iets gevonden wordt
        try {
            # 1. Probeer de site te vinden als actieve site. Als gevonden, probeer dan te soft-verwijderen.
            ...
            if ($checkActiveSite) {
                ...
                $siteStillExists = $true # Het is nu in de prullenbak, dus nog niet "vrij"
            }
            
            # 2. Controleer nu de verwijderde sites (prullenbak). Als gevonden, probeer dan permanent te verwijderen.
            # Deze check is belangrijk, zelfs als er net een soft-delete is uitgevoerd.
            $checkDeletedSite = Get-SPODeletedSite -Identity $fullDesiredSharePointSiteUrl_Algemeen -ErrorAction SilentlyContinue
            if ($checkDeletedSite) {
                Write-Warning "Site '$fullDesiredSharePointSiteUrl_Algemeen' gevonden in SharePoint Prullenbak (2e fase). Probeert deze permanent te verwijderen..."
                ...
                $siteStillExists = $true # Na een permanente verwijdering moet de lus nog een keer draaien om te bevestigen dat hij echt weg is.
            }

            # Als na de pogingen de site nog steeds actief of in de prullenbak is, dan wachten.
            # Dit is een secundaire check die bevestigt of na de remove-pogingen de site echt weg is.
            # De loop stopt pas als Get-SPOSite en Get-SPODeletedSite *beide* niets meer vinden.
            if ($siteStillExists) { # Als een van de bovenstaande checks de site nog vond (of succesvol heeft geprobeerd te verwijderen)
            ...
            # Als na deze ronde geen actieve of verwijderde site is gevonden, is de site vrij.
            ...
            $retryCountForClearance++
            $siteStillExists = $true # Als er een fout optreedt, aannemen dat de site nog steeds bestaat en het opnieuw proberen
        }
    }
    ...
    Write-Warning "BELANGRIJK: Zorg ervoor dat het account waarmee u nu inlogt (indien gevraagd) minimaal de rol 'SharePoint Administrator' heeft in Microsoft 365. Voltooi eventuele inlogprompts." 
...
} ### NIEUWE ACCCOLADE HIER TOEGEVOEGD (voor de hoofd try-blok) ###
catch {
    ...
    # GEEN RESET VAN OMGEVINGSVARIABELE WANT DIE WAS NIET IN JOUW ORIGINELE SCRIPT
