...
Import-Module -Name Microsoft.Online.SharePoint.PowerShell -ErrorAction Stop -verbose
Import-Module Microsoft.Graph.Authentication -ErrorAction Stop -verbose
...
# Je moet de code kopiëren en plakken naar microsoft.com/devicelogin in een webbrowser.
...
try {
    ...
    Write-Warning "U krijgt een code om in te loggen via microsoft.com/devicelogin. Voltooi deze in een browser." # Aangepaste waarschuwing
    ...
    try {
        ...
        Start-Sleep -Seconds 60 # Verlaagd van 90 naar 60 seconden
    ...
    Write-Host "Opnieuw verbinding maken met SharePoint Online Admin Center voor URL-validatie..." -ForegroundColor Yellow
    Write-Warning "BELANGRIJK: Zorg ervoor dat het account waarmee u nu inlogt (via apparaatcode) minimaal de rol 'SharePoint Administrator' heeft in Microsoft 365. Voltooi de inlogprompt met de code." # Aangepaste waarschuwing
    
    try {
        Connect-SPOService -Url $adminUrl -ErrorAction Stop 
        Write-Host "Verbinding met SharePoint Online Admin Center hersteld." -ForegroundColor Green
    } catch {
        throw "Kritieke fout: Kan geen verbinding maken met SharePoint Online Admin Center ($adminUrl) om URL-beschikbaarheid te controleren. Controleer uw rechten en inlogproces. Oorzaak: $($_.Exception.Message)"
    }
    ...
    while($siteStillExists -and $retryCountForClearance -lt $maxRetryForClearance) { 
        $retryCountForClearance++ # Verplaats de increment hier, aan het begin van de lus
        Write-Host "  Poging $($retryCountForClearance)/$($maxRetryForClearance): Controleren op site '$fullDesiredSharePointSiteUrl_Algemeen'..."
        
        # Vlag om te bepalen of we moeten wachten voor de volgende poging
        $needsDelayForNextAttempt = $false 

        try {
            # Initialiseer $checkActiveSite en $checkDeletedSite voor deze iteratie
            $checkActiveSite = $null
            $checkDeletedSite = $null

            # 1. Controleer of de URL nog bezet is door een OUDE actieve site. Zo ja, probeer deze te soft-verwijderen.
            $checkActiveSite = Get-SPOSite -Identity $fullDesiredSharePointSiteUrl_Algemeen -ErrorAction SilentlyContinue
            if ($checkActiveSite) {
                Write-Warning "Site '$fullDesiredSharePointSiteUrl_Algemeen' is nog steeds actief. Probeert deze nu te verwijderen (soft-delete)..."
                Remove-SPOSite -Identity $fullDesiredSharePointSiteUrl_Algemeen -Confirm:$false -ErrorAction Stop
                Write-Host "Actieve site '$fullDesiredSharePointSiteUrl_Algemeen' succesvol verwijderd (naar prullenbak)." -ForegroundColor Green
                $needsDelayForNextAttempt = $true # We hebben een actie uitgevoerd, dus wacht op propagatie
            }
            
            # 2. Controleer nu de SharePoint prullenbak (2e fase) voor de URL. Zo ja, probeer deze permanent te verwijderen.
            $checkDeletedSite = Get-SPODeletedSite -Identity $fullDesiredSharePointSiteUrl_Algemeen -ErrorAction SilentlyContinue
            if ($checkDeletedSite) {
                Write-Warning "Site '$fullDesiredSharePointSiteUrl_Algemeen' gevonden in SharePoint Prullenbak. Probeert deze permanent te verwijderen..."
                Remove-SPODeletedSite -Identity $fullDesiredSharePointSiteUrl_Algemeen -Confirm:$false -ErrorAction Stop
                Write-Host "Site '$fullDesiredSharePointSiteUrl_Algemeen' permanent verwijderd uit SharePoint Prullenbak." -ForegroundColor Green
                $needsDelayForNextAttempt = $true # We hebben een actie uitgevoerd, dus wacht op propagatie
            }

            # 3. Bepaal of de site nu echt vrij is van OUDE sites
            if (-not $checkActiveSite -and -not $checkDeletedSite) {
                # Als de URL nu vrij is van OUDE sites (na eventuele verwijdering hierboven), dan is de lus hier klaar.
                $siteStillExists = $false
            } else {
                # De site is nog steeds actief of in de prullenbak (of net verplaatst naar prullenbak)
                $siteStillExists = $true 
            }

        } catch {
            Write-Warning "Fout bij controleren/verwijderen URL beschikbaarheid: $($_.Exception.Message)"
            $needsDelayForNextAttempt = $true # Bij een fout willen we ook een delay voordat we opnieuw proberen
            $siteStillExists = $true # Bij fout, aannemen dat site nog bestaat en doorgaan met pogingen
        }

        # Pas een vertraging toe als dat nodig is voor de volgende poging, en we nog niet klaar zijn met de lus
        if ($needsDelayForNextAttempt -and $siteStillExists -and $retryCountForClearance -lt $maxRetryForClearance) {
    ...
    if ($siteStillExists) { # Als de lus is gestopt omdat max retries zijn bereikt en de site nog steeds bestaat
    ...
    # NIEUW: Diagnosticeringspunt voor het aanmaken van de groep
    Write-Host "`nDEBUG: Start poging tot aanmaken van Microsoft 365 groep '$algemeenGroupName'." -ForegroundColor DarkYellow
    $newAlgemeenGroup = $null # Initialiseer dit om te controleren of de groep echt is aangemaakt
    try {
        $newAlgemeenGroup = New-MgGroup -DisplayName $algemeenGroupName `
                                     ...
                                     -ErrorAction Stop # Belangrijk: stop hier als er een fout is

        if ($newAlgemeenGroup) {
            Write-Host "Groep '$algemeenGroupName' succesvol opnieuw aangemaakt (ID: $($newAlgemeenGroup.Id))." -ForegroundColor Green
            Start-Sleep -Seconds 5 # Korte pauze na aanmaken van groep
        } else {
            # Dit pad zou in theorie niet bereikt moeten worden met -ErrorAction Stop
            throw "New-MgGroup gaf geen object terug voor '$algemeenGroupName', hoewel ErrorAction Stop was ingesteld."
        }
    } catch {
        # Vang de specifieke fout op bij het aanmaken van de groep
        ...
        throw # Werp de fout opnieuw om de hoofd try-catch te activeren en het script te stoppen
    }
