...
$domainname = "supracomlab3.onmicrosoft.com"
...
$balieUsers = 1..2 | ForEach-Object { "Balie$_" }
...
$adminUrl = "https://$Domain-admin.sharepoint.com" # Bijv. https://supracombv-admin.sharepoint.com

# Hier wordt de URL omgezet (Oude URL van Algemeen wordt omgeleid). Dit is de URL die vrij moet komen.
$oldUrlSharePointSite = "https://$Domain.sharepoint.com/sites/algemeen" # Bijv. https://supracombv.sharepoint.com/sites/algemeen
$newUrlSharePointSite = "https://$Domain.sharepoint.com/sites/willbedeleted" # Bijv. https://supracombv.sharepoint.com/sites/willbedeleted (Site waar de inhoud van oude Algemeen naar verhuist)
...
$fullDesiredSharePointSiteUrl_Algemeen = "https://$Domain.sharepoint.com/sites/$algemeenGroupMailNickname" # Let op: dit is dezelfde URL als $oldUrlSharePointSite
...
# Vraag om bevestiging Y/N (Y of N verplicht)
do {
    $reinstall = Read-Host -Prompt "Wilt u Microsoft.Graph modules (her)installeren? (Y/N)"
    if ($null -eq $reinstall) { $reinstall = '' }
    $reinstall = $reinstall.Trim().ToUpper()
...
if ($reinstall -eq 'Y') {
    ...
    Write-Host "Installatie overgeslagen door gebruiker. Er wordt geprobeerd bestaande modules te importeren." -ForegroundColor Cyan
}
...
try {
    ...
    #Dit stuk script leid de oude URL van Algemeen om en verwijdert deze indien nodig
    ...
    try {
        ...
        # Verwijderen van de oude site als deze nog steeds bestaat (in geval van omleiding of niet gevonden)
    ...
    # Verbreek de SPO Service verbinding
...
foreach ($user in $personalUsers) {
    ...
    $siteValidated = $false
    $urlCheckAttempts = 0
    $maxUrlCheckAttempts = 3 # Kan even duren voordat de site live is, 3 pogingen = 30 seconden
    ...
    while (-not $siteValidated -and $urlCheckAttempts -lt $maxUrlCheckAttempts) {
        ...
        try {
            # Controleer of de SharePoint site bestaat en of de URL klopt
...
# Zorgt ervoor dat de omgeleide site verwijderd wordt
Write-Host "`nVerwijderen van de omgeleide SharePoint site '$newUrlSharePointSite'..." -ForegroundColor Yellow

$removeSiteValidated = $false
$removeUrlCheckAttempts = 0
$maxRemoveUrlCheckAttempts = 3 # Aantal pogingen om te controleren/verwijderen

while (-not $removeSiteValidated -and $removeUrlCheckAttempts -lt $maxRemoveUrlCheckAttempts) {
    $removeUrlCheckAttempts++
    try {
        $siteToRemove = Get-SPOSite -Identity $newUrlSharePointSite -ErrorAction SilentlyContinue
        if ($siteToRemove) {
            Write-Warning "Omgeleide site '$newUrlSharePointSite' gevonden. Probeer nu te verwijderen (poging $removeUrlCheckAttempts/$maxRemoveUrlCheckAttempts)..."
            Remove-SPOSite -Identity $newUrlSharePointSite -Confirm:$false -ErrorAction Stop
            Write-Host "Omgeleide site '$newUrlSharePointSite' succesvol verwijderd." -ForegroundColor Green
            $removeSiteValidated = $true
            Start-Sleep -Seconds 5 
        } else {
            Write-Host "Omgeleide site '$newUrlSharePointSite' niet gevonden, geen verwijdering nodig." -ForegroundColor Cyan
            $removeSiteValidated = $true
        }
    } catch {
        Write-Warning "Fout bij verwijderen van omgeleide site '$newUrlSharePointSite': $($_.Exception.Message)"
        Write-Warning "Controleer handmatig of de omgeleide site nu is verwijderd."
        if ($removeUrlCheckAttempts -lt $maxRemoveUrlCheckAttempts) {
            Write-Host "Wachten en opnieuw proberen..." -ForegroundColor Yellow
            Start-Sleep -Seconds 10
        }
    }
